...
export class ComputeComponent implements OnInit {
  ...
  identMatrix: number[][] = [[1,0,0],[0,1,0],[0,0,1]]
  matrix: number[][] = [[2,3,1],[3,2,1],[2,6,4]];
  answer: number[][] = []
  ngOnInit(): void {
  ...
  computeResult() {
    this.service.performCompute(this.identMatrix, this.matrix).subscribe(
      data => {
        this.hasResult = true;
        console.log("Result: ", this.hasResult)
        this.answer = data.matrix;
      }
    )
  }
}
